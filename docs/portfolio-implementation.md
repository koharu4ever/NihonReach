...
质量门禁包含前端格式、Vue/TypeScript 类型检查、生产构建、Pint、PHPStan 和 PHPUnit。部署准备增加了异常输入、重复关闭、分页、可信代理和管理员命令测试；最新验收范围见 [Coolify 部署准备说明](coolify-deployment.md)，具体测试数字以对应版本的命令输出为准。
...
- 已实现 VPS 部署、生产邮件、对象存储、独立 API、Redis 或微服务。
