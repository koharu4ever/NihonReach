# NihonReach：Coolify 部署准备与人工验收

本文件是操作说明，不代表已经执行公网部署。项目仍是求职 Portfolio Demo，不是真实企业运营系统。所有实际域名、生产密码和 APP_KEY 只填入 Coolify，不写入本文件。
...
- 未执行 Git 提交/推送，未操作 VPS、Coolify 或 DNS；公网证书、浏览器交互和备份恢复仍未验收。

## 3. Coolify 准备：先确认，再操作

以下步骤尚未代你执行。先审阅本轮 Diff 和测试结果，再由你决定提交、推送及首次部署。
...
只有完成第 5 节后，才可以增加：“通过 Coolify 部署到个人 VPS，配置域名、HTTPS、运行时密钥及数据库备份恢复。”不要把这份说明本身当作已经部署的证据。
